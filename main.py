...
class Parser():
    def __init__(self,url,requests,proxies=None):
        ...
        try:
            for request in self.requests:
                with sync_playwright() as pl:
                    ...
                    self.browser = pl.chromium.launch(headless=False,proxy=self.proxies)
                    ...
                    self.page.get_by_placeholder('Поиск и выбор мест').type(text=request,delay=1)
    ...
    def get_result(self):
        index = min([len(data_list) for data_list in self.data.values()])
        pruned_data = {key: val[:index] for key, val in self.data.items()}
        return pruned_data
...
if __name__ == '__main__':
    parser = Parser(url='https://yandex.ru/maps',requests=['Ярославль больницы'])
    parser.parse()
