...
def window():
    ...
    def addExport():
        ...
        export_pieces.append( (export_dir.get() , export_name.get() , format_converter[formats.get()]) )
    ...
    def changeExport():
        ...
        export_pieces[change_idx] = (export_dir.get() , export_name.get() , format_converter[formats.get()])
    ...
    def deleteExport():
        ...
        del export_pieces[change_idx]
        export_variables.set(new_exports)

    def run_and_stop():
        cur_text = run_btn.cget('text')
        variable = multiprocessing.sharedctypes.Value('b', True)
        if cur_text == 'Старт':
    ...
    proxy.insert(0,'Отсутствует')
    proxy.place(x=20,y=15)
    addPlaceholder(proxy,'укажите путь к .json прокcи конфигурации')
    ...
    formats = ttk.Combobox(window,values=['.xlsx(Excel)','.csv','.db(SQLite)'],state='readonly',font=16)
    formats.current(0)
